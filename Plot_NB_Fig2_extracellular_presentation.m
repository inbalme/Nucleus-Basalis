...
evoked_trace_Off_f23_pos(1,:) = [0.08 , 0.83 , 0.4 , 0.1];
evoked_trace_Off_f23_pos(2,:) = [0.08 , 0.5 , 0.4 , 0.1]; evoked_trace_Off_f23_pos(2,2)=evoked_trace_Off_f23_pos(1,2)-evoked_trace_Off_f23_pos(2,4)-0.04;
...
evoked_trace_On_f23_pos(1,:) = [0.08 , 0.5 , 0.4 , 0.1];  evoked_trace_On_f23_pos(1,2)=evoked_trace_Off_f23_pos(2,2)-evoked_trace_On_f23_pos(1,4)-0.08;
evoked_trace_On_f23_pos(2,:) = [0.08 , 0.5 , 0.4 , 0.1]; evoked_trace_On_f23_pos(2,2)=evoked_trace_On_f23_pos(1,2)-evoked_trace_On_f23_pos(2,4)-0.04;
...
h_dist=evoked_trace_Off_f23_pos(1,1)+evoked_trace_Off_f23_pos(1,3)+0.1;
evoked_trace_Off_f33_pos(1,:) = [h_dist , evoked_trace_Off_f23_pos(1,2) ,  evoked_trace_Off_f23_pos(1,3),  evoked_trace_Off_f23_pos(1,4)];
evoked_trace_Off_f33_pos(2,:) = [h_dist , evoked_trace_Off_f23_pos(2,2) ,  evoked_trace_Off_f23_pos(2,3) ,  evoked_trace_Off_f23_pos(2,4)];
evoked_trace_Off_f33_pos_top = evoked_trace_Off_f23_pos(1,2)+evoked_trace_Off_f23_pos(1,4);

evoked_trace_On_f33 = open('file33 whisker evoked activity NB+.fig');    
evoked_trace_On_f33_ax = get(gcf, 'children');
evoked_trace_On_f33_pos(1,:) = [h_dist , evoked_trace_On_f23_pos(1,2) ,  evoked_trace_Off_f23_pos(1,3) ,  evoked_trace_On_f23_pos(1,4)];
evoked_trace_On_f33_pos(2,:) = [h_dist , evoked_trace_On_f23_pos(2,2) ,  evoked_trace_Off_f23_pos(2,3) ,  evoked_trace_On_f23_pos(2,4)];
...
h_dist=0.08;
vert_dim=0.16;
Response_modulation_pos(1,:) = [evoked_trace_Off_f23_pos(1,1) , evoked_trace_On_f33_pos(2,2)-vert_dim-0.11, 0.11 , vert_dim];
Response_modulation_pos_top = Response_modulation_pos(1,2)+Response_modulation_pos(1,4);

%SNR Index
SNR = open('SNR.fig');    
SNR_ax = get(gcf, 'children');
SNR_pos(1,:) = [Response_modulation_pos(1,1)+Response_modulation_pos(1,3)+h_dist , Response_modulation_pos(1,2), 0.11 , vert_dim];
SNR_pos_top = SNR_pos(1,2)+SNR_pos(1,4);

%Success Rate
Success_Rate = open('Success Rate.fig');    
Success_Rate_ax = get(gcf, 'children');
Success_Rate_pos(1,:) = [SNR_pos(1,1)+SNR_pos(1,3)+h_dist  , Response_modulation_pos(1,2), 0.11 , vert_dim];
Success_Rate_pos_top = Success_Rate_pos(1,2)+Success_Rate_pos(1,4);

%Latency
Latency = open('Latency.fig');    
Latency_ax = get(gcf, 'children');
Latency_pos(1,:) = [Success_Rate_pos(1,1)+Success_Rate_pos(1,3)+h_dist , Response_modulation_pos(1,2), 0.11 , vert_dim];
Latency_pos_top = Latency_pos(1,2)+Latency_pos(1,4);

%Jitter (Latency SD)
Latency_STD = open('Latency STD.fig');    
Latency_STD_ax = get(gcf, 'children');
Latency_STD_pos(1,:) = [Latency_pos(1,1)+Latency_pos(1,3)+h_dist , Response_modulation_pos(1,2), 0.11 , vert_dim];
Latency_STD_pos_top = Latency_STD_pos(1,2)+Latency_STD_pos(1,4);
...
set(gcf,'PaperUnits','centimeters','PaperPosition',[1.2 1.2 21 20]); %[left, bottom, width, height] 
...
set(evoked_trace_Off_f23_ax_copy(2),'position',evoked_trace_Off_f23_pos(1,:),...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5, 'ylim', y2limits(1,:),'ytick', y2ticks(1,:));
   set(F, 'currentaxes', evoked_trace_Off_f23_ax_copy(2));  t=title(''); ylabel('Trial', 'fontsize', 13)
 ...
 set(F, 'currentaxes', evoked_trace_Off_f23_ax_copy(1)); xl=xlabel('');  t=title(''); ylabel('Spike/S', 'fontsize', 13); %h = get(gca, 'ylabel'); set(h, 'fontsize', 11)
...
set(evoked_trace_On_f23_ax_copy(2),'position',evoked_trace_On_f23_pos(1,:),...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5, 'ylim', y2limits(1,:),'ytick', y2ticks(1,:));
   set(F, 'currentaxes', evoked_trace_On_f23_ax_copy(2));   t=title(''); ylabel('Trial', 'fontsize', 13)
  ...
  set(F, 'currentaxes', evoked_trace_On_f23_ax_copy(1)); t=title('');  ylabel('Spike/S', 'fontsize', 13); xlabel('Time [S]', 'fontsize', 13);
...
set(evoked_trace_Off_f33_ax_copy(2),'position',evoked_trace_Off_f33_pos(1,:),...
    ...
    set(F, 'currentaxes', evoked_trace_Off_f33_ax_copy(2));  t=title(''); ylabel('Trial', 'fontsize', 13)
 ...
 set(F, 'currentaxes', evoked_trace_Off_f33_ax_copy(1)); t=title('');  xl=xlabel(''); ylabel('Spike/S', 'fontsize', 13)
...
set(evoked_trace_On_f33_ax_copy(2),'position',evoked_trace_On_f33_pos(1,:),...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5, 'ylim', y4limits(1,:),'ytick', y4ticks(1,:));
    set(F, 'currentaxes', evoked_trace_On_f33_ax_copy(2));  t=title(''); ylabel('Trial', 'fontsize', 13)
   ...
   set(F, 'currentaxes', evoked_trace_On_f33_ax_copy(1)); t=title('');ylabel('Spike/S', 'fontsize', 13); xlabel('Time [S]', 'fontsize', 13);
...
set(SNR_ax_copy,'position',SNR_pos(1,:),'ylim',[-0.1 1.1] ,'ytick', [0 0.5 1],...
 ...
 %Success Rate
 Success_Rate_ax_copy = copyobj(Success_Rate_ax,F); % copy axes to new fig
set(Success_Rate_ax_copy,'position',Success_Rate_pos(1,:),'ylim',[-0.1 1.1] ,'ytick', [0 0.5 1],...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5,'box','off'); %, 'ylim', y6limits,'ytick', y6ticks,'xlim',x6limits,'xtick',x6ticks, 'xticklabel',x6ticklab);
 set(F, 'currentaxes', Success_Rate_ax_copy); t=title(''); yl=ylabel('Success Rate','fontsize',13); %set Ytick to be on the left side and not on the right

 %Latency
 Latency_ax_copy = copyobj(Latency_ax,F); % copy axes to new fig
set(Latency_ax_copy,'position',Latency_pos(1,:),'ylim',[5 25] ,'ytick', [5 15 25],...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5,'box','off'); %, 'ylim', y6limits,'ytick', y6ticks,'xlim',x6limits,'xtick',x6ticks, 'xticklabel',x6ticklab);
 set(F, 'currentaxes', Latency_ax_copy); t=title(''); yl=ylabel('Latency [mS]','fontsize',13); %set Ytick to be on the left side and not on the right

  %Jitter (Latency SD)
Latency_STD_ax_copy = copyobj(Latency_STD_ax,F); % copy axes to new fig
set(Latency_STD_ax_copy,'position',Latency_STD_pos(1,:),'ylim',[0 7] ,'ytick', [0 3 6],...
       'fontname', 'arial','fontsize',ax_fontsize,'linewidth',1.5,'box','off'); %, 'ylim', y6limits,'ytick', y6ticks,'xlim',x6limits,'xtick',x6ticks, 'xticklabel',x6ticklab);
 set(F, 'currentaxes',Latency_STD_ax_copy); t=title(''); yl=ylabel('Jitter [mS]','fontsize',13); %set Ytick to be on the left side and not on the right

% %annotations:
% annotation('textbox', [spont_trace_Off_f23_pos(1,1) spont_trace_Off_f23_pos_top 0 0]+[-0.01 0.02 0.5 0.05],...
...
%   annotation('textbox', [Success_Rate_pos(1,1) Success_Rate_pos_top 0 0]+a_pos1,...
%      'FitHeightToText', 'on', 'edgecolor', 'none','string', 'K', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
%   annotation('textbox', [Latency_pos(1,1) Latency_pos_top 0 0]+a_pos1,...
%      'FitHeightToText', 'on', 'edgecolor', 'none','string', 'L', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
%   annotation('textbox', [Latency_STD_pos(1,1) Latency_STD_pos_top 0 0]+a_pos1,...
