...
h_dist=spont_trace_Off_f23_pos(1,1)+spont_trace_Off_f23_pos(1,3)+0.05;
...
h_dist=spont_trace_Off_f33_pos(1,1)+spont_trace_Off_f33_pos(1,3)+0.05;
...
vert_dim=0.16;
Response_modulation_pos(1,:) = [h_dist , evoked_trace_Off_f23_pos_top-vert_dim , 0.12 , vert_dim];
Response_modulation_pos_top = Response_modulation_pos(1,2)+Response_modulation_pos(1,4);

%SNR Index
SNR = open('SNR.fig');    
SNR_ax = get(gcf, 'children');
SNR_pos(1,:) = [h_dist , Response_modulation_pos(1,2)-vert_dim-0.06, 0.12 , vert_dim];
...
vert_dim=0.14;
Success_Rate_pos(1,:) = [h_dist , SNR_pos(1,2)-vert_dim-0.07, 0.12 , vert_dim];
...
vert_dim=0.16;
Latency_pos(1,:) = [h_dist-0.05 , Success_Rate_pos(1,2)-vert_dim-0.07, 0.09 , vert_dim];
...
vert_dim=0.16;
Latency_STD_pos(1,:) = [Latency_pos(1,1)+0.09+0.06 , Latency_pos(1,2), 0.09 , vert_dim];
...
filename='file23 whisker evoked activity NB-';
