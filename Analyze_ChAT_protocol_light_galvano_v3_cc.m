%% Analyze ChAT protocol light+galvano v3 - recent version.
% This file was created on 17/10/2016 and based on the file "Analyzy NBES protocol ES+galvano v3"
%This file is used for the analysis of files created with extract_ChAT_Data_v3

%       protocol 5 (laser+galvnao (5 x-values))
%       protocol 6 (ChAT intracellular current injections light+galvnao (15 x-values))
%       protocol 8 - ChAT intracellular: light only (3 x-values): 1) train 20 Hz, 10ms ondur, 50 pulses. 2) single pulse 5 sec ondur. 3)single pulse 10 ms
...
% before ES
 if trace_type==2;
         ...
         data_Vm_plot=data_no_spikes{1}(interval_plot(:,1),trace,x_value(1));
...
Fig{fileind}(trace_type)=figure;
    hold on
        ...
        if trace_type==2;
            ...
            l=size(patch_xdata,2)-1;
            patch_ydata=wextend('ac','sym',yex,l,'r');
...
% after ES
  if trace_type==2;
         ...
         data_Vm_plot=data_no_spikes{1}(interval_plot(:,2),trace,x_value(2));
