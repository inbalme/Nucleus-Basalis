...
% This file was created on 5/10/2015 and updated on 7/6/2016
...
save_flag= 0;
print_flag=0;
data_vec_all = []; data_vec_residual_all = [];  %[8,10,12,14,15,16,22,36,37,40,1,46,48,50,52,56,58,72];
...
% %         fn_shadedErrorBar([1:size(plot_data,1)].*dt,plot_data_mean(:,:,3),plot_data_std(:,:,3),{'LineWidth',1,'color', color_table(2,:)},1);
% % 
...
%     data_deriv2=diff(data_mean_smooth,2);
    ...
    %taking the 50 ms prior to the first stim:
                data_pre_response(:,:,x_value) = data_no_spikes{channel}(stim2_X{2}(1,1)-int_time.*sf{1}./1000:stim2_X{2}(1,1)-1,:,x_value);
    ...
    %taking the response as the whole train:  
                ...
                data_train_response(:,:,x_value) = data_no_spikes{channel}(train_interval,:,x_value);
        ...
        %looking at the 400ms after the train:
                ...
                data_post_train_response(:,:,x_value) = data_no_spikes{channel}(post_train_interval,:,x_value);
...
for trace=1:size(data_train_response,2);
    noise2(:,trace)=data_no_spikes{channel}(start_sample-duration.*sf{1}:start_sample-1,trace,x_value);
...
SNR2=(Amplitude_signal/Amplitude_noise2)^2;
        ...
        % global baseline (100ms before the onset of stim. train):
                ...
                baseline_global(:,:,x_value) = data_no_spikes{channel}(baseline_global_interval,:,x_value);
           ...
           for stim_num=1:size(stim2_X{x_value},2);
                ...
                response_end=response_begin+int_time.*sf{1}./1000-1;  
                %taking 50 ms of response according to stim. number in the train:
                data_response(:,:,x_value) = data_no_spikes{channel}(response_begin:response_end,:,x_value);
                ...
                baseline_local_std(stim_num,:) = std(data_no_spikes{channel}(baseline_interval,:,x_value),0,1); %std along time, within the trace
                baseline_local_average(1,:) = mean(data_no_spikes{channel}(baseline_interval,:,x_value),1); %mean along time, within the trace
...
% h1=figure;        
%         hold on
%         errorbar([1:size(peak_amp{2}(:,:),2)], nanmean(peak_amp{2}(:,:),1),nanstd(peak_amp{2}(:,:),0,1),'k-', 'LineWidth',1.5,'marker','o','markerfacecolor',color_table(1,:))
%          errorbar([1:size(peak_val{3}(:,:),2)], nanmean(peak_amp{3}(:,:),1),nanstd(peak_amp{3}(:,:),0,1),'color',color_table(2,:), 'LineWidth',1.5,'marker','o','markerfacecolor',color_table(2,:))
%         ylim_data=[get(gca,'ylim')]';
%         p_amp=max(ylim_data).*ones(size(s_amp));
%         plot(s_amp,p_amp,'k*')
%         hold off
...
hold off
        ...
        title(['Global Baseline, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_baseline_global)] ,'FontSize', 20,'fontname', 'arial'); 
...
errorbar(adapt_amp_X(:,1), mean(adapt_amp_Y,2),E,'k','linewidth',2.5,'markersize',10,'markerfacecolor','k')
...
hold off
        ...
        title(['Adaptation Amplitude ratio, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_adapt_amp)] ,'FontSize', 20,'fontname', 'arial');   
...
hold off
        ...
        title(['Adaptation power ratio, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_F1)] ,'FontSize', 20,'fontname', 'arial');   
...
hold off
        ...
        title(['pre-train STD, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_pre_response_STD)] ,'FontSize', 20,'fontname', 'arial');   
...
hold off
        ...
        'ticklength', [0.010 0.010],'fontname', 'arial','xticklabel',{'NB-','NB+'} ,'box', 'off'); %'fontweight', 'bold', 
        ylabel('std [mV]', 'FontSize', 28,'fontname', 'arial');
        title(['evoked train STD, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_Vm_res_train_STD)] ,'FontSize', 20,'fontname', 'arial');   
...
hold off
        ...
        'ticklength', [0.010 0.010],'fontname', 'arial','xticklabel',{'NB-','NB+'} ,'box', 'off'); %'fontweight', 'bold', 
        ylabel('std [mV]', 'FontSize', 28,'fontname', 'arial');
        title(['post-train STD, n=' num2str(length(files_to_analyze)) ', p=' num2str(peaks_stat(stim_num).wilcoxon_p_post_train_STD)] ,'FontSize', 20,'fontname', 'arial');   
...
clear color_table
    ...
    stim_num=1;
   j1=figure;  
    capincrease=(-0.06); %#units to increase length of errorbar cap
    asymmetric=1;
    barwidth1=0.3;
        hold on
