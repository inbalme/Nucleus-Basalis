...
function [code result] = fn_RemoveSpikesMO2(voltages, dt)
    % RemoveSpikes2 does one direction, so we use it twice.
    [code r] = RemoveSpikes2(voltages, dt);
    r = -1 * r;
    [d r] = RemoveSpikes2(r, dt);
    result = -1 * r;
...
function [code result] = RemoveSpikes2(voltages, dt,spike_derivative_threshold)
    if nargin<3;
