function Extract_NBES_Data_v3
% function Extract_NBES_Data_v3 forms the database 'Extracted Data' used by
...
for fileind =[51]; %[17,23,26,27,28,29,33,35,60]; %[31,38,42,51,61,64,67,69,71,74,77]; %[16,22,44,46,48,50,52]; %[32,34,36,37,38,39]
...
if isempty(protocol)
    ...
    switch protocol
        ...
        case 5
             ...
             ES_del = Param.facade(11);
             ES_ondur = Param.facade(12);
             ES_nstim = Param.facade(13);
             ES_freq = Param.facade(14);
        case 6
...
%              galvano_flag(1,:) = [0 0 0 0 0 0 0];  
        ...
        case 11
            ...
            ES_del = Param.facade(22);
            ES_ondur = Param.facade(23);
            ES_nstim = Param.facade(24);
            ES_freq = Param.facade(25);
...
%              ES_duration_time = Param. ES_param_val(5)*(1/Param. ES_param_val(4)); %number of pulses times 1/f
    if sum(ES_flag)==0
             ...
             ES_begin_time = ES_del;
             ES_duration_time = ES_nstim*(1/ES_freq); %number of pulses times 1/f
             ES_end_time = ES_begin_time+ES_duration_time;
             ES_vec{ii}(:,1) = zeros(length(time_axis{ii}),1);
             ES_vec{ii}(ES_begin_time*sf{ii}:ES_end_time*sf{ii},1)=1;
             
             locations_x_ES{ii}(1,:) = ES_begin_time*sf{ii}; %arranging the galvano begin and end locations in one variable for plotting
             locations_x_ES{ii}(2,:) = ES_end_time*sf{ii}; 
    end
...
%%
 name = files(fileind).extracted_name;
    ...
    save( name, 'Param','ES_flag','color_table', 'data','raw_data','code', 'data_no_spikes','galvano_flag','stim1_X','stim2_X', 'stim2_time','time_axis')  % for specific parameter from Exp look at GetPatameters
