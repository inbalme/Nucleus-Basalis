...
function [F,obj] = fn_Plot_Trace(raw_data,dt_data, dt_stim, stim_X);
...
%         'fontname', 'myriad', 'fontweight', 'bold', 'box', 'off' );
        xlabel('Time [sec]' ,'FontSize', 20,'fontname', 'myriad');
        ylabel('Vm [mV]', 'FontSize', 20,'fontname', 'myriad');
       set(gca, 'fontsize',20, 'fontname', 'arial', 'box', 'off','linewidth',1) 
        

hold on
for i = 1:size(raw_data,2);
    obj(i)=plot((1:size(raw_data,1)).*dt_data,raw_data(:,i), 'LineWidth',0.2,'color', color_table(i,:));
end
if nargin == 4
